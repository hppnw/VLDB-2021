...
**GitHub仓库地址：** https://github.com/hppnw/VLDB-2021
...
在Lab 1实现Raft协议时，HandleRaftReady函数中持久化与消息发送的顺序问题导致测试失败。Raft协议要求持久化必须在消息发送之前完成，否则节点崩溃重启后可能出现其他节点已收到日志但本地未持久化的不一致状态。解决方案是严格遵循处理顺序：先调用SaveReadyState持久化所有状态变更，再发送Messages，最后应用CommittedEntries。

### 幂等性实现问题

Lab 2和Lab 3中，Prewrite、Commit、Rollback等操作需要处理网络重传带来的重复请求。初始实现中，重复的Prewrite请求返回错误导致测试失败。Percolator协议要求所有操作具备幂等性：重复的Prewrite应直接返回成功，重复的Commit应检查Write记录状态后返回成功。通过检查锁的所有权和Write记录的存在性，实现了操作的幂等性。

### 并行Projection死锁问题

Lab 4C实现并行Projection时，资源池的循环依赖导致死锁：Fetcher等待Worker归还Input Chunk，Worker等待主线程消费Output Chunk，主线程等待Fetcher提供数据。解决方案包括：调整Channel缓冲大小以打破循环等待，规范资源归还顺序，引入显式的globalOutputCh关闭机制确保数据流完整处理。

### 不确定错误处理问题

Lab 3中，Primary Key提交时发生RPC超时，后续重试遇到Region错误时，若清除RPC错误状态可能导致重复提交破坏幂等性。解决方案是引入Undetermined Error状态：当Primary Key提交发生RPC超时且无法确认状态时，设置Undetermined Error标志，仅在成功返回且无Region错误时才清除该标志，确保错误状态正确传递。

### MVCC锁可见性判断

Lab 2实现Get命令时，锁的可见性判断需要遵循快照隔离语义：lock.Ts <= startTS的锁对当前事务可见，需要等待或返回Locked错误；lock.Ts > startTS的锁对当前事务不可见，可忽略继续读取。这保证了事务读取开始时刻的一致性视图。

### Primary Key提交语义

Lab 3实现Commit阶段时，Primary Key提交成功后，即使Secondary Keys提交失败，事务仍被视为成功。这是Percolator协议的核心设计：Primary Key的提交状态决定整个事务状态，后续读操作通过检查Primary Key状态确认事务已提交，Secondary Locks通过ResolveLock机制异步清理。

### 实现局限性

当前实现存在以下局限性：Lab 4C的并行Projection中Worker数量固定，未根据数据量和系统负载动态调整；Lab 1的快照传输未实现断点续传，网络中断需重新传输；测试场景主要覆盖网络超时和Region错误，未涉及节点永久故障、网络分区、时钟偏差等场景。
